...
class Population:
    ...
    def print(self):
        ...
        for c in sortedList:
            print(
                f"Composição: {c.composition} | Peso: {c.weight} | Valor: {c.value} | %peso limite {c.usedWeightPercent} | fitness (aptidão): {c.fitness}")
        print(f" Média de peso da população é {self.weightAverage} kg")
        print()
    ...
    def procreate(self, ancestral: object):
        ...
        if (self.procreateMethod == "random"):
            for i in range(self.size):
                ...
                new_cromossomo = self.n_point_crossover(p1, p2)
    ...
    # Método novo: Adicionado considerando que o crossover que estava sendo utilizado (1_point) não era muito bom.
    def n_point_crossover(self, p1: object, p2: object) -> object:
        '''Realiza cruzamento (crossover) de n pontos entre dois pais'''
        
        n = random.randint(2, 5) # Escolhe o número de "cortes" aleatoriamente.
        assert len(p1) == len(p2), "Verifica se os pais devem ter o mesmo comprimento"
        assert n > 0, "Verifica se n (numéro de cortes) é maior que 0"
 
        tam = len(p1)
        pontos = random.sample(range(1, tam), n)
        pontos.sort()
        nova_composicao = []
        prev_point = 0
        ...
        for i, point in enumerate(pontos):
            if i % 2 == 0:
                child = p1[prev_point:point] + p2[point:]
            else:
                child = p2[prev_point:point] + p1[point:]
            nova_composicao.append(child)
            prev_point = point
        
        c = Cromossomo()
        c.composition = nova_composicao
        c.evaluate_fitness()
