...
class Cromossomo:
    ...
    def mutate(self):
        ...
        if (self.mutateMethod == "fix"):
            ...
            self.rand_mutate()
        elif (self.mutateMethod == "uniform"):
            self.uniform_mutate()
        self.evaluate_fitness()  # atualiza características do cromossomo
    ...
    # Método novo: Uma mutação que pode abranger todos os genes do cromossomo.
    def uniform_mutate(self):
        '''Faz a mutação uniforme com uma probabilidade de mutação dada'''
        mutation_rate = 0.01
        for i in range(self.compositionSize):
